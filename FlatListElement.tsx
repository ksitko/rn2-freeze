...
const FlatListElement = ({ text, onPress, selected }: FlatListElementProps) => {
  const open = useSharedValue(selected);
  const progress = useDerivedValue(() =>
    open.value ? withSpring(1, { damping: 14, stiffness: 130 }) : withSpring(0, { damping: 14, stiffness: 130 })
  );
      ...
      {selected && (
        <TouchableWithoutFeedback onPress={onPress}>
          <View style={[styles.alt, styles.container]}>
            <Text style={styles.text}>ALT {text}</Text>
          </View>
        </TouchableWithoutFeedback>
      )}
    </Animated.View>
